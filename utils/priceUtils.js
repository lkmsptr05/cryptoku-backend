...
        
        if (idrErr) console.warn("Warning: Failed to fetch IDR rate from DB, using fallback 15500.");

        // 2. Ambil harga native token (BNB, ETH, MATIC) dari crypto_prices
        const nativeSymbol = getNativeSymbolForNetwork(networkKey); 
        // Asumsi symbol di crypto_prices adalah ETHUSDT, BNBUSDT, dsb.
...
        const NATIVE_PRICE_USD = nativePriceData?.price_usd ? Number(nativePriceData.price_usd) : 0; 

        if (priceErr || NATIVE_PRICE_USD === 0) {
            console.error(`Failed to fetch native price for ${nativeSymbol}:`, priceErr);
