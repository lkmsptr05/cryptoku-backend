...
export class GasEstimator {
  ...
  async _fetchNativeUsdPrice(priceSymbol) {
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      console.warn("crypto_prices fetch failed for", priceSymbol, ":", error?.message);
      return null;
    }
  ...
  async _getCachedPrices(chainId) {
    ...
    // Check cache
    if (cache.ts + cache.ttl > now && cache.nativeUsd != null && cache.usdIdr != null) {
    ...
    if (nativeUsd == null || usdIdr == null) {
      console.warn("Price fetch returned nulls, using 0:", { nativeUsd, usdIdr, priceKey });
    }

    return { nativeUsd: this._priceCache.nativeUsd, usdIdr: this._priceCache.usdIdr };
  }
  ...
  async _getGasPrice() {
    ...
    const priority = feeData.maxPriorityFeePerGas ?? feeData.maxFeePerGas ?? feeData.gasPrice ?? ethers.parseUnits("1", "gwei");
  ...
  async estimate({ from, to, tokenAddress = null, amount = null }) {
    ...
    try {
      if (tokenAddress) {
        ...
        // PERBAIKAN KRUSIAL: Gunakan amount || "0" (nol) untuk estimasi gas.
        // Simulasi transfer 0 tidak akan gagal karena "exceeds balance".
        const data = token.interface.encodeFunctionData("transfer", [to, amount || "0"]);
        
        gasLimit = await this.provider.estimateGas({ from, to: tokenAddress, data });
      } else {
